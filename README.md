...
- **Fallback path 2:** local sparse placeholder tree with deterministic ASCII-safe materialization when both ProjFS and native WebDAV mounting are unavailable.
...
- if the native Windows WebDAV drive mount fails, the app falls back again to a sparse local placeholder tree using deterministic ASCII-safe names so the full tree can still be materialized reliably inside Explorer
