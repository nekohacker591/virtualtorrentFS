...
8. Falls back to a cache-aware local tree plus an in-process WebDAV endpoint if ProjFS is unavailable on the machine.
...
- if ProjFS is unavailable, the app keeps the local fallback tree focused on directories plus already-cached payload files to reduce Explorer disk churn
- in the same fallback mode, the app also starts a localhost WebDAV endpoint so Windows WebDAV clients or tools such as Mountain Duck can browse the full metadata namespace without pre-materializing every file on disk
