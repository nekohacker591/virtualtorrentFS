...
8. Falls back to a cache-aware sparse placeholder tree that preserves each file's logical size without eagerly consuming the full payload on disk if ProjFS is unavailable on the machine.
...
- if ProjFS is unavailable, the app falls back to a cache-aware sparse placeholder tree mounted to the requested drive letter so media players still observe the expected logical file sizes
