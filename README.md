...
6. Attempts to serve file data from the local payload cache when files are read.
7. Falls back to a cache-aware sparse on-disk placeholder tree if ProjFS is unavailable on the machine.
...
The Windows mount/materialization code now attempts to handle non-Latin torrent paths more robustly by decoding torrent path bytes as UTF-8 first, then falling back to the active Windows ANSI code page, and sanitizing characters that Windows does not allow in file names.
