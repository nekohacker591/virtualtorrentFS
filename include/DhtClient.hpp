...
class DhtClient {
  public:
    ...
    [[nodiscard]] std::vector<std::uint8_t> buildGetPeersQuery(std::string_view txId, const std::array<std::uint8_t, 20>& infoHash) const;
    [[nodiscard]] std::vector<std::uint8_t> buildAnnouncePeerQuery(std::string_view txId, const std::array<std::uint8_t, 20>& infoHash, std::uint16_t port, std::string_view token) const;
