...
void TorrentEngine::start() {
    ...
    (void)portMapper_.buildUpnpAddPortMappingSoap(6881, 6881, "127.0.0.1");
    (void)portMapper_.buildUpnpDeletePortMappingSoap(6881);
    (void)dht_.buildPingQuery("aa");
    (void)dht_.buildFindNodeQuery("ab", peerId_);
    (void)dht_.buildGetPeersQuery("ac", peerId_);
    (void)dht_.buildAnnouncePeerQuery("ad", peerId_, 6881, "torrentfs");
}
