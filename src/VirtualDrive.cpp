...
std::wstring normalizeProjectedPath(PCWSTR filePathName) {
    if (filePathName == nullptr || *filePathName == L'\0') {
        return L"/";
    }

    std::wstring out = L"/";
    for (const wchar_t ch : std::wstring(filePathName)) {
...
std::wstring relativeProjectedPathFor(const std::string& torrentRelativePath, bool forceAscii = false) {
    std::wstring out;
    std::size_t start = 0;
    while (start < torrentRelativePath.size()) {
        ...
        out += (forceAscii ? asciiFallbackComponent(token) : sanitizeWindowsComponent(torrentTextToWide(token)));
...
std::wstring projectedLookupKey(const std::string& torrentRelativePath, bool forceAscii = false) {
    return L"/" + relativeProjectedPathFor(torrentRelativePath, forceAscii);
}
...
std::wstring webDavRemoteName(std::uint16_t port) {
    return L"\\\\127.0.0.1@" + std::to_wstring(port) + L"\\DavWWWRoot\\";
}
...
std::filesystem::path materializedPath(const std::filesystem::path& root, const std::string& torrentRelativePath, bool forceAscii = false) {
    ...
    const auto relativeProjected = relativeProjectedPathFor(torrentRelativePath, forceAscii);
    std::size_t start = 0;
    while (start < relativeProjected.size()) {
        const auto slash = relativeProjected.find(L'/', start);
        path /= relativeProjected.substr(start, slash == std::wstring::npos ? std::wstring::npos : slash - start);
...
bool ensureSparsePlaceholderFile(const std::filesystem::path& path, std::uint64_t size) {
    ...
    logicalSize.QuadPart = static_cast<LONGLONG>(size);
    const bool sized = SetFilePointerEx(file, logicalSize, nullptr, FILE_BEGIN) != 0 && SetEndOfFile(file) != 0;
    ...
    SetFileAttributesW(widePath.c_str(), FILE_ATTRIBUTE_READONLY | FILE_ATTRIBUTE_OFFLINE | FILE_ATTRIBUTE_SPARSE_FILE);
    return true;
}
...
class WindowsProjFsProvider final : public VirtualDriveProvider {
  public:
    int mount(const Config& config, std::shared_ptr<TorrentSession> session) override {
        sessionHolder_ = std::move(session);
        session_ = sessionHolder_.get();
        root_ = config.cacheDirectory / ".projfs-root";
        buildProjectedNameIndex();
        ...
        if (FAILED(hr) && hr != HRESULT_FROM_WIN32(ERROR_REPARSE_TAG_MISMATCH)) {
            std::cerr << "PrjMarkDirectoryAsPlaceholder failed: 0x" << std::hex << static_cast<unsigned long>(hr)
                      << ". Falling back to cache-aware placeholder materialization.\n";
            startWebDavFallback();
            if (tryMountWebDavDrive(config)) {
                return 0;
            }
    ...
    ~WindowsProjFsProvider() override {
        if (webDavMapped_) {
            const std::wstring deviceName = std::wstring(1, mappedDriveLetter_) + L":";
            WNetCancelConnection2W(deviceName.c_str(), CONNECT_UPDATE_PROFILE, TRUE);
        }
        webDav_.stop();
  ...
  private:
    ...
    int mapDriveLetter(const Config& config, bool projFsActive) {
        ...
        std::cout << "Mounted namespace at " << std::string(1, config.driveLetter) << ":\\ using "
                  << (projFsActive ? "Windows ProjFS" : "ascii-safe sparse placeholder fallback") << ".\n"
    ...
    void buildProjectedNameIndex() {
        projectedToVirtual_.clear();

        projectedToVirtual_[L"/"] = "/";
        for (const auto& file : session_->metadata().files()) {
            registerProjectedPath(file.relativePath);
        }
    }

    void registerProjectedPath(const std::string& torrentRelativePath) {
        std::string currentVirtual;
        std::size_t start = 0;
        while (start < torrentRelativePath.size()) {
            const auto slash = torrentRelativePath.find('/', start);
            const auto token = torrentRelativePath.substr(start, slash == std::string::npos ? std::string::npos : slash - start);
            if (!currentVirtual.empty()) {
                currentVirtual.push_back('/');
            }
            currentVirtual += token;
            projectedToVirtual_.emplace(projectedLookupKey(currentVirtual, false), "/" + currentVirtual);
            projectedToVirtual_.emplace(projectedLookupKey(currentVirtual, true), "/" + currentVirtual);
            if (slash == std::string::npos) {
                break;
            }
            start = slash + 1;
    ...
    std::optional<FileEntry> lookupProjectedEntry(PCWSTR filePathName) const {
        const auto projectedPath = normalizeProjectedPath(filePathName);
        if (const auto it = projectedToVirtual_.find(projectedPath); it != projectedToVirtual_.end()) {
            return session_->lookup(it->second);
        }
        return std::nullopt;
    }

    std::vector<FileEntry> listProjectedDirectory(PCWSTR filePathName) const {
        const auto projectedPath = normalizeProjectedPath(filePathName);
        if (const auto it = projectedToVirtual_.find(projectedPath); it != projectedToVirtual_.end()) {
            return session_->listDirectory(it->second);
        }
        return {};
    }
        ...
        for (unsigned int t = 0; t < threadCount; ++t) {
            workers.emplace_back([&]() {
                while (!failed.load()) {
                    ...
                    const auto rootFile = materializedPath(root_, record.relativePath, true);
    ...
    void startWebDavFallback() {
        if (webDav_.running() || !sessionHolder_) {
            return;
        }
        if (webDav_.start(sessionHolder_)) {
            std::cout << "WebDAV fallback available at " << webDav_.endpoint() << "\n"
                      << "Windows WebDAV UNC (WebClient): \\\\127.0.0.1@" << webDav_.port() << "\\DavWWWRoot\\\n"
                      << "If Windows cannot mount it directly, open the same endpoint in Mountain Duck.\n";
        } else {
            std::cerr << "Failed to start WebDAV fallback server.\n";
        }
    }

    bool tryMountWebDavDrive(const Config& config) {
        if (!webDav_.running()) {
            return false;
        }

        std::wstring deviceName = std::wstring(1, static_cast<wchar_t>(config.driveLetter)) + L":";
        NETRESOURCEW resource{};
        resource.dwType = RESOURCETYPE_DISK;
        std::wstring remoteName = webDavRemoteName(webDav_.port());
        resource.lpLocalName = deviceName.data();
        resource.lpRemoteName = remoteName.data();

        const DWORD result = WNetAddConnection2W(&resource, nullptr, nullptr, CONNECT_TEMPORARY);
        if (result == NO_ERROR) {
            webDavMapped_ = true;
            mappedDriveLetter_ = static_cast<wchar_t>(config.driveLetter);
            std::cout << "Mounted namespace at " << std::string(1, config.driveLetter)
                      << ":\\ using native Windows WebDAV fallback.\n";
            return true;
        }

        std::cerr << "Native WebDAV drive mount failed with error " << result
                  << ". Falling back to local sparse placeholders.\n";
        return false;
    }
    ...
    static HRESULT CALLBACK getDirEnum(const PRJ_CALLBACK_DATA* callbackData,
        ...
        if (callbackData->Flags & PRJ_CB_DATA_FLAG_ENUM_RESTART_SCAN || state.entries.empty()) {
            state.entries = provider->listProjectedDirectory(callbackData->FilePathName);
    ...
    static HRESULT CALLBACK getPlaceholderInfo(const PRJ_CALLBACK_DATA* callbackData) {
        auto* provider = self(callbackData);
        const auto entry = provider->lookupProjectedEntry(callbackData->FilePathName);
        if (!entry) {
            return HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND);
        }

        if (!entry->isDirectory) {
            provider->session_->beginStreaming(*entry, 0, 0);
        }
    ...
    static HRESULT CALLBACK getFileData(const PRJ_CALLBACK_DATA* callbackData, std::uint64_t byteOffset, std::uint32_t length) {
        ...
        provider->session_->beginStreaming(*entry, byteOffset, length);
        ...
        for (int attempt = 0; attempt < 1200; ++attempt) {
            ...
            if (haveData && bytesRead == 0 && !provider->session_->isHydrating(*entry)) {
                break;
            }
    ...
    std::shared_ptr<TorrentSession> sessionHolder_;
    ...
    std::unordered_map<std::wstring, std::string> projectedToVirtual_;
    WebDavServer webDav_;
    bool webDavMapped_ = false;
    wchar_t mappedDriveLetter_ = L'\0';
};

} // namespace
#endif

int NullVirtualDriveProvider::mount(const Config& config, std::shared_ptr<TorrentSession> session) {
    std::cout
