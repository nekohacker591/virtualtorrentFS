...
std::filesystem::path materializedPath(const std::filesystem::path& root, const std::string& torrentRelativePath) {
    ...
    while (start < torrentRelativePath.size()) {
        ...
        path /= sanitizeWindowsComponent(torrentTextToWide(token));
        ...
        for (unsigned int t = 0; t < threadCount; ++t) {
            workers.emplace_back([&]() {
                while (!failed.load()) {
                    ...
                    const auto rootFile = materializedPath(root_, record.relativePath);
