...
TorrentMetadata TorrentMetadata::loadFromFile(const std::filesystem::path& path) {
    ...
    if (const auto announceIt = root.find("announce"); announceIt != root.end() && announceIt->second.isString()) {
        metadata.announceUrl_ = announceIt->second.asString();
    }
    ...
    metadata.pieceHashes_.resize(metadata.pieceCount_);
    for (std::size_t i = 0; i < metadata.pieceCount_; ++i) {
        std::memcpy(metadata.pieceHashes_[i].data(), piecesRaw.data() + (i * 20), 20);
    }
...
const std::string& TorrentMetadata::announceUrl() const { return announceUrl_; }
...
const std::vector<std::array<std::uint8_t, 20>>& TorrentMetadata::pieceHashes() const { return pieceHashes_; }
